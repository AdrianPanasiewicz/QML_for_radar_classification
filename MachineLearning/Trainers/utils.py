...
def square_kernel_matrix(X, kernel, assume_normalized_kernel=False):
    ...
    **Example:**
    ...
    num_off_diagonal = (N * (N - 1)) // 2
    num_diagonal = 0 if assume_normalized_kernel else N
    total_iterations = num_off_diagonal + num_diagonal

    with tqdm(total=total_iterations, desc="Computing kernel matrix") as pbar:
...
def kernel_matrix(X1, X2, kernel):
    ...
    **Example:**
    ...
    with tqdm(total=total_iterations, desc="Computing kernel matrix") as pbar:
        for x, y in product(X1, X2):
            matrix_list.append(kernel(x, y))
            pbar.update(1)

    matrix = math.stack(matrix_list)
